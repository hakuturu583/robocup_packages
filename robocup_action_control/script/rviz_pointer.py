...
class state_drawer():
    def __init__(self):
        self.pub_text = rospy.Publisher(rospy.get_namespace()+"/state",OverlayText,queue_size=10)
        self.msg_text = OverlayText()
        self.msg_text.fg_color.r = 0
        self.msg_text.fg_color.g = 0.8
        self.msg_text.fg_color.b = 1.0
        self.msg_text.fg_color.a = 1.0
        self.msg_text.action = self.msg_text.ADD
        self.msg_text.text_size = 30.0
        self.msg_text.width = 500
        self.msg_text.height = 500
        self.msg_text.left = 0
        self.msg_text.top = 300

    def draw(self,state):
        self.msg_text.text = "nao_head_control status =>" + state
        self.pub_text.publish(self.msg_text)
...
class head_controler:
    def __init__(self):
        ...
        self.gain = 0.3
        self.state_text = state_drawer()
        self.seq = 1

    def run(self):
        if self.is_initialized == True:
            self.state_text.draw(self.state)
            if self.state == "init":
                self.init()
            if self.state == "stop":
                self.stop()
