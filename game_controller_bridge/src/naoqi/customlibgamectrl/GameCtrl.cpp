...
private:
  ...
  AL::ALTextToSpeechProxy* speech;/**< Give access to ALTextToSpeech. */
        ...
        if(chestButtonPressed != previousChestButtonPressed && now - whenChestButtonStateChanged >= BUTTON_DELAY)
        {
          if(chestButtonPressed && whenChestButtonStateChanged) // ignore first press, e.g. for getting up
          {
            ...
            if(player.penalty == PENALTY_NONE)
            {
              ...
              if(now - whenPacketWasReceived < GAMECONTROLLER_TIMEOUT && send(GAMECONTROLLER_RETURN_MSG_MAN_PENALISE))
              {
                whenPacketWasSent = now;
                speech->say("Penalized");
              }
            }
            else
            {
              player.penalty = PENALTY_NONE;
              if(now - whenPacketWasReceived < GAMECONTROLLER_TIMEOUT && send(GAMECONTROLLER_RETURN_MSG_MAN_UNPENALISE))
              {
                whenPacketWasSent = now;
                speech->say("Not Penalized");
              }
              else
...
public:
    ...
    teamNumber(0)
  {
    ...
    try
    {
      speech = new AL::ALTextToSpeechProxy(pBroker);
